...
================================================================================
1. 启动方式：
   - 方式 1（直接双击，推荐）：直接双击根目录下的 CarrierVision.exe 即可秒级启动（原生无黑框启动器）。
   - 方式 2（环境变量一键注册）：双击运行 register_env.bat 即可一键将本程序的 lib/ 目录注册至用户 PATH；如需注销可随时双击 unregister_env.bat。
   - 方式 3（便携启动脚本）：双击运行 CarrierVision.bat 启动。

2. 运行库说明：
   - 绿色便携包已收纳所需依赖。若在未安装 Visual C++ 运行库的纯净 Windows 客户机上遇到运行库缺失，可双击根目录下附带的 vc_redist.x64.exe 快速安装。
================================================================================
